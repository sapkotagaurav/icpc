...
public:
  ...
  void djikstra(int start)
  {
    ...
    for (int i=0; i<adjList.size();i++)
    {
      ...
      {
        if (dis[adjList[temp.first][i].first] > dis[temp.first] + adjList[temp.first][i].second)
        {
          p.push(adjList[temp.first][i]);
          dis[adjList[temp.first][i].first] = dis[temp.first] + adjList[temp.first][i].second;
        }
      }
    }
    for (auto &&i : dis)
